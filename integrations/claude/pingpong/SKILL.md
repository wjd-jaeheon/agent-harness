...
8. `AWAIT_PLAN_APPROVAL`이면 반환된 `runId`, `currentPlanPath`, `currentPlanSha`를 사용한다. `currentPlanPath`는 상대 경로이므로 `D:/codex-projects/agent-harness/.harness/runs/<runId>/<currentPlanPath>`에서 최종 PLAN을 읽어 한국어로 보여주고 승인·보완·취소 중 하나를 명시적으로 묻는다.
9. 사용자의 명시적 선택에만 대응하는 기존 `approve-plan`, `request-plan-revision`, `run`, `abort` 명령을 호출한다. exact `runId`와 PLAN SHA를 사용한다.
10. `NEEDS_HUMAN`이면 `lastError`를 그대로 보여주고 멈춘다. 실패한 provider를 임의로 우회하거나 새 run을 만들지 않는다.
