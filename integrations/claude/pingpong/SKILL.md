...
새 작업 진입 시점부터 최종 SPEC 요약에 대한 사용자의 명시적 승인 전에는 현재 Claude가 직접 대화하고 읽기 전용 조사만 한다. 임시 파일 생성, `harness.mjs`의 `init`·`run`·`start` 호출, Cursor Scout, 다른 provider, subagent는 호출하지 않는다.
...
9. 다음 공개 runner 명령으로 run만 만든다. `init`은 provider를 하나도 부르지 않으므로 여기서 SPEC을 고쳐도 비용이 들지 않는다. 명령이 끝나면 임시 SPEC을 삭제하고 반환된 `runId`를 기억한다. runner가 필요한 원본은 run 안의 `SPEC.md`로 이미 잠근다.

```powershell
node "D:/codex-projects/agent-harness/harness.mjs" init --repo "<현재 Git root>" --spec "<작성한 SPEC 파일>"
```

10. `init` 결과의 `specCommandBaseline`과 `specAcceptanceCoverage`를 표로 보여준다. 이건 SPEC이 실제로 무엇을 검증하는지에 대한 기계적 사실이므로 계획 핑퐁에 돈을 쓰기 전에 반드시 사람이 보게 한다.
    ...
    - `exit_code`가 `null`이면 명령이 실행조차 되지 않았다는 뜻이므로 `error`를 그대로 보여주고 명령을 고칠지 묻는다.
    ...
    - 사용자가 SPEC을 고치기로 하면 이 run을 `abort`한 뒤 5번으로 돌아간다. 아직 provider를 한 번도 부르지 않았으므로 버리는 비용이 없다.
11. 사용자가 baseline을 확인하고 그대로 진행하겠다고 하면, 그때 같은 `runId`로 계획 핑퐁을 실행하고 사람 gate까지 기다린다.

```powershell
node "D:/codex-projects/agent-harness/harness.mjs" run --run "<runId>"
```
...
```powershell
node "D:/codex-projects/agent-harness/harness.mjs" init --repo "<현재 Git root>" --spec "<작성한 SPEC 파일>" --parent-run "<이전 runId>"
```
