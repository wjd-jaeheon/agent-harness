...
- define at least one ordered checkpoint using this exact four-line machine-readable form: `CP-001: title`, `Paths: relative/file, another/file`, `ACs: AC-001`, `Commands: CMD-001`. Keep each field on one line, use comma-separated exact relative file paths without globs, cover every AC and CMD across the checkpoints, and number checkpoints sequentially. Every CP, AC, and CMD ID MUST use exactly three digits with a zero-padded number (`CP-001`, `CP-002`, `CP-010`, …), never a single- or double-digit form like `CP-1`.
